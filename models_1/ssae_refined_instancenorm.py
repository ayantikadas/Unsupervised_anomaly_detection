...
class custom_AE(nn.Module):
    ...
    def __init__(self, encode_layers_info,decode_layers_info):
        ...
        self.dropout = torch.nn.AlphaDropout(p=0.2)
        ...
        self.dropout_bt = torch.nn.AlphaDropout(p=0.2)
        
        # Decode
        self.post_bottleneck_layer = decode_bt_layer_list[0]
        ...
        self.final_layer  = self.conv2d_block(in_channels=decode_layers_info['output_channels'][-1], out_channels=1, kernel_size = 1,stride = 1,padding = 0,norm_ = False, activation_ = False)
    
    def forward(self, x):
        # Encode
        encode_block1 = self.conv_encode1(x)
        encode_block11 = self.dropout(encode_block1)
        encode_block2 = self.conv_encode2(encode_block11)
        encode_block3 = self.conv_encode3(encode_block2)
        # Bottleneck
        bt = self.bottleneck_layer(encode_block3)
        bt1 = self.dropout_bt(bt)
        # Decode
        cat_layer5 = self.post_bottleneck_layer(bt1)        
...
class custom_AE2(nn.Module):
    ...
    def __init__(self, encode_layers_info,decode_layers_info):
        ...
        self.final_layer  = self.conv2d_block(in_channels=32, out_channels=1, kernel_size = 1,stride = 1,padding = 0,norm_ = False, activation_ = False)
